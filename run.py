...
def main():
    ...
    depth_model_size = "small"  # Depth Anything v2 model size: "small", "base", "large" // Zoedepth ['train', 'infer', 'eval']
    ...
    # Detection settings
    conf_threshold = 0.1  # Confidence threshold for object detection
    ...
    try:
        depth_estimator = DepthEstimator(
            model_size=depth_model_size,  # "infer" for ZoeDepth, "small"/"base"/"large" for Depth Anything
            device=device,
            backend='depth-anything'  # or 'depth-anything'  //'zoedepth' 
        )
    except Exception as e:
    ...
    cap = cv2.VideoCapture('/home/khled/Desktop/Thesis/Depth/Depth-Anything-V2- Yolo/fire_person_test (online-video-cutter.com).mp4')
    ...
    if enable_bev:
        ...
        bev = BirdEyeView(scale=60, size=(300, 300), camera_image_size=(width, height))  # Increased scale to spread objects out
    ...
    while True:
        ...
        try:
            ...
            # Make a deep pristine copy BEFORE any processing
            ...
            try:
                detection_frame, detections, tracking_trajectories = detector.detect(detection_frame, track=enable_tracking)
            except Exception as e:
            ...
            # Collect person boxes for fire interaction check
            person_boxes = [b['bbox_2d'] for b in boxes_3d if 'human' in b['class_name'].lower()]

            # Draw boxes with intersection logic
            for box_3d in boxes_3d:
                try:
                    ...
                    if 'fire' in class_name:
                        # Check if fire intersects with any person
                        intersecting = bbox3d_estimator.check_intersection(x1, y1, x2, y2, person_boxes)
                        color = (0, 0, 255) if intersecting else (0, 255, 0)  # Red if intersects, green if safe x
                        box_3d['intersecting'] = intersecting
                        fire_x, fire_y = bbox3d_estimator.find_fire_target_from_largest_blob(pristine_frame, int(x1), int(x2), int(y1), int(y2))
                        bbox3d_estimator.draw_target_marker_at(result_frame, fire_x, fire_y, color)
                        
                    elif 'human' in class_name:
                        color = (255, 0, 0)  # Green for person
                    else:
                        color = (255, 255, 255)  # White for other objects

                    result_frame = bbox3d_estimator.draw_box_3d(result_frame, box_3d, color=color)
                    result_frame = bbox3d_estimator.draw_tracking_trajectories(result_frame, tracking_trajectories)
                except Exception as e:
            ...
            # cv2.putText(result_frame, f"{fps_display} | Device: {device}", (10, height - bev_height - 20), 
            #            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
